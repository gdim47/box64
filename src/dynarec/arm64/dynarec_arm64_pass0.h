...
#define JUMP(A, C)         add_jump(dyn, ninst); add_next(dyn, (uintptr_t)A); if(!box64_dynarec_jcond_disable_barriers){SMEND();} dyn->insts[ninst].x64.jmp = A; dyn->insts[ninst].x64.jmp_cond = C; dyn->insts[ninst].x64.jmp_insts = 0
